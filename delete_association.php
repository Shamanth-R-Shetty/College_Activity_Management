...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($action === "showDetails") {
        ...
        $associationQuery = "SELECT association_name, association_icon FROM association WHERE association_id = ?";
        $associationStatement = $conn->prepare($associationQuery);
        $associationStatement->bind_param("s", $associationId);
        $associationStatement->execute();
        $associationResult = $associationStatement->get_result();
        ...
        if ($associationResult->num_rows > 0) {
            ...
            $studentQuery = "SELECT student_id, student_name FROM student WHERE association_id = ?";
            $studentStatement = $conn->prepare($studentQuery);
            $studentStatement->bind_param("s", $associationId);
            ...
            $activityQuery = "SELECT activity_id, activity_name FROM activity WHERE association_id = ?";
            $activityStatement = $conn->prepare($activityQuery);
            $activityStatement->bind_param("s", $associationId);
            ...
            $staffQuery = "SELECT staff_id, staff_name FROM staff WHERE association_id = ?";
            $staffStatement = $conn->prepare($staffQuery);
            $staffStatement->bind_param("s", $associationId);
            $staffStatement->execute();
            $staffResult = $staffStatement->get_result();
        ...
        if (empty($associationId)) {
            ...
            $deleteAttendanceQuery = "DELETE FROM attendance WHERE activity_id IN (SELECT activity_id FROM activity WHERE association_id = ?)";
            $deleteAttendanceStatement = $conn->prepare($deleteAttendanceQuery);
            $deleteAttendanceStatement->bind_param("s", $associationId);
            $deleteAttendanceStatement->execute();

            $deleteAttendanceQuery = "DELETE FROM attendance WHERE student_id IN (SELECT student_id FROM student WHERE association_id = ?)";
            $deleteAttendanceStatement = $conn->prepare($deleteAttendanceQuery);
            $deleteAttendanceStatement->bind_param("s", $associationId);
            $deleteAttendanceStatement->execute();

            // Delete associated records from activity, staff, and student tables
            $deleteActivityQuery = "DELETE FROM activity WHERE association_id = ?";
            $deleteActivityStatement = $conn->prepare($deleteActivityQuery);
            $deleteActivityStatement->bind_param("s", $associationId);
            $deleteActivityStatement->execute();

            $deleteStaffQuery = "DELETE FROM staff WHERE association_id = ?";
            $deleteStaffStatement = $conn->prepare($deleteStaffQuery);
            $deleteStaffStatement->bind_param("s", $associationId);
            $deleteStaffStatement->execute();

            $deleteStudentQuery = "DELETE FROM student WHERE association_id = ?";
            $deleteStudentStatement = $conn->prepare($deleteStudentQuery);
            $deleteStudentStatement->bind_param("s", $associationId);
            $deleteStudentStatement->execute();

            // Delete association record from association table
            $deleteAssociationQuery = "DELETE FROM association WHERE association_id = ?";
            $deleteAssociationStatement = $conn->prepare($deleteAssociationQuery);
            $deleteAssociationStatement->bind_param("s", $associationId);
            $deleteAssociationStatement->execute();
...
<head>
    ...
    <style>
        ...
        .container {
            ...
            margin-top: 100px; /* Adjust the margin-top value as per your preference */
        }
        .container .title{
          font-size: 25px;
          font-weight: 500;
